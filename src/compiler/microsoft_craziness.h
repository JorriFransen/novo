...
void find_visual_studio_by_fighting_through_microsoft_craziness(Windows_SDK_Info *result) {
    ...
    while (1) {
        ...
        if (os_file_exists(library_file)) {
            result->vs_tools_path = concat(bstr_inst_path, L"\\VC\\Tools");
            auto link_exe_path = concat(result->vs_tools_path, L"\\MSVC\\", version, L"\\bin\\Hostx64\\x64");
