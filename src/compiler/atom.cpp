...
void initialize_atoms(Allocator* allocator, s64 capacity)
{
    ...
    g_atoms.hashes = nallocate_size(allocator, capacity * (sizeof(g_atoms.hashes[0]) + sizeof(g_atoms.ids[0])), u64);
    ...
    g_atoms.first_sting_block = nallocate_size(allocator, string_block_size, Atom_String_Memory_Block);
...
void free_atoms()
{
    nrelease(g_atoms.allocator, g_atoms.hashes);

    auto sb = g_atoms.first_sting_block;
    while (sb) {
        auto next = sb->next_block;

        nrelease(g_atoms.allocator, sb);
...
static void atom_table_grow()
{
    ...
    auto new_hashes = nallocate_size(g_atoms.allocator, new_cap * (sizeof(g_atoms.hashes[0]) + sizeof(g_atoms.ids[0])), u64);
    ...
    nrelease(g_atoms.allocator, old_hashes);
}
...
static char* atom_table_add_string(const String_Ref &str)
{
    ...
    if (required > remaining) {
        ...
        if (ex_block) {
            ...
            auto new_block = nallocate_size(g_atoms.allocator, total_size, Atom_String_Memory_Block);
