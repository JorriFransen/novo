...
void instance_init(Instance* inst, Options options)
{
    ...
    inst->temp_allocator = temp_allocator_create(&inst->temp_allocator_data, default_alloc, MEBIBYTE(24));
    inst->ast_allocator = linear_allocator_create(&inst->ast_allocator_data, default_alloc, MEBIBYTE(24));
    ...
    while (progress) {
        ...
        for (s64 i = 0; i < inst->run_tasks.count; i++) {
            ...
            if (task->node.kind == AST_Node_Kind::EXPRESSION) {
                ...
                Type_Task type_task = type_task_create(inst, ast_node(gen_expr), infer_node(run_result.type), task->scope, nullptr, nullptr);
